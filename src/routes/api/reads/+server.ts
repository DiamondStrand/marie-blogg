...
export const GET: RequestHandler = async ({ url }) => {
	...
	const cacheKey = `${spaceId}|${ids.sort().join(',')}`;
	...
	const where: string[] = ["event = 'item_read'", `properties['item_id'] IN (${inList})`];
	if (spaceId) {
		where.push(`properties['space_id'] = '${(spaceId as string).replace(/'/g, "''")}'`);
	}
	const hogql = `
		SELECT properties['item_id'] AS item_id, COUNT(DISTINCT distinct_id) AS readers
		FROM events
	...
	try {
		...
		if (!res.ok) {
			...
			console.error('[reads] PostHog query failed', res.status, text);
			...
			const tokenType = TOKEN.startsWith('phc_')
				...
				: TOKEN.startsWith('phx_') || TOKEN.startsWith('phs_') || TOKEN.startsWith('phsa_')
					? 'personal/service'
					: 'unknown';
			console.error('[reads] Config check', {
				host: HOST,
				projectId: PROJECT,
			...
			console.error(
				'[reads] Hint: Use Personal API Key/Service Account (not phc_ project key), set numeric POSTHOG_PROJECT_ID, and HOST=https://eu.posthog.com'
			);
			throw new Error('PostHog query failed');
		}
		const data = await res.json();
		// Handle both flat and nested PostHog response shapes
		// Shape A: { results: [[...]], columns: [..] }
		// Shape B: { results: { results: [[...]], columns: [..] } }
		// Shape C: { results: [{ item_id, readers }, ...] }
		...
		for (const row of rows) {
			if (Array.isArray(row)) {
				if (columns) {
					...
					const idxReaders = columns.indexOf('readers');
					const item_id = idxId >= 0 ? row[idxId] : row[0];
					const readers = idxReaders >= 0 ? row[idxReaders] : row[1];
					out[item_id] = Number(readers) || 0;
				} else {
					const [item_id, readers] = row;
					out[item_id] = Number(readers) || 0;
				}
			} else if (row && typeof row === 'object') {
				out[(row as any).item_id] = Number((row as any).readers) || 0;
			}
		}
